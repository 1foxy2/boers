...
public class BoreHeadModelWrapper implements ItemModel {
    ...
    public BoreHeadModelWrapper(List<ItemTintSource> tints, QuadCollection quads, ModelRenderProperties properties, Matrix4fc transformation) {
        this.tints = tints;
        this.quads = quads;
    ...
    public record Unbaked(Identifier model, Optional<Transformation> transformation, List<ItemTintSource> tints) implements ItemModel.Unbaked {
        public static final MapCodec<BoreHeadModelWrapper.Unbaked> MAP_CODEC = RecordCodecBuilder.mapCodec(
            i -> i.group(
                    Identifier.CODEC.fieldOf("model").forGetter(BoreHeadModelWrapper.Unbaked::model),
                    Transformation.EXTENDED_CODEC.optionalFieldOf("transformation").forGetter(BoreHeadModelWrapper.Unbaked::transformation),
                    ItemTintSources.CODEC.listOf().optionalFieldOf("tints", List.of()).forGetter(BoreHeadModelWrapper.Unbaked::tints)
                )
                .apply(i, BoreHeadModelWrapper.Unbaked::new)
        );
        ...
        public ItemModel bake(BakingContext context, Matrix4fc transformation) {
            ...
            BoreHeadModelWrapper.validateAtlasUsage(quads.getAll());
            Matrix4fc modelTransform = Transformation.compose(transformation, this.transformation);
            return new BoreHeadModelWrapper(this.tints, quads, properties, modelTransform);
        }

        @Override
        public MapCodec<BoreHeadModelWrapper.Unbaked> type() {
            return MAP_CODEC;
        }
    }
}
