...
public class FixerRegistryFixedCodec<E> implements Codec<Holder<E>> {
    private final ResourceKey<? extends Registry<E>> registryKey;

    public static <E> FixerRegistryFixedCodec<E> create(ResourceKey<? extends Registry<E>> registryKey) {
        return new FixerRegistryFixedCodec<>(registryKey);
    }

    private FixerRegistryFixedCodec(ResourceKey<? extends Registry<E>> registryKey) {
                ...
                return ResourceLocation.CODEC
                        ...
                        .flatMap(
                                p_337524_ -> {
                                    ...
                                    Optional<Holder.Reference<E>> optionalHolder = optional.get()
                                            .get(ResourceKey.create(this.registryKey, resourcelocation));

                                    if (optionalHolder.isEmpty() && resourcelocation.getNamespace().equals("boers")) {
                                        optionalHolder = optional.get()
                                                .get(ResourceKey.create(this.registryKey, Utils.rl(resourcelocation.getPath())));
                                    }
    ...
    public String toString() {
        return "FixerRegistryFixedCodec[" + this.registryKey + "]";
    }
}
